...
$(function() {	
	...
	$(".basketListItem").each(function() {
		...
		$(this).find(".orderPrice").text(orderPrice);
		$(this).find("input[name='orderPrice']").val(orderPrice);
		
		let freedelivery = $(this).nextAll(".sellerItem").first().find(".freedelivery").text();
				  		...
				  		<li class="list-group-item basketListItem" data-goodsNo="${vo.goodsNo }" data-basketno="${vo.basketNo }">
				  			...
				  			<div class="row">
				  				...
				  				<div class="col-md-2 dataRow">
				  					<!-- 상품 이미지 -->
			  						<img class="rounded goodsImage img-fluid" src="${vo.goodsMainImage }" data-goodsiamge="${vo.goodsMainImage }" alt="상품이미지" width="150" height="150">
			  					</div>
				  				<div class="col-md-4">	 
				    				...
				    				<!-- 결제 금액 배송비 -->
				    				<h5><b class="orderPrice"></b> <small class="dlvyCharge">${vo.merchant_delivery }</small></h5>
				    				<input type="hidden" value="" name="orderPrice" class="orderData">
				    				<input type="hidden" value="${vo.merchant_delivery }" name="dlvyCharge" class="orderData">	  
				    				<br>	    				
			  					</div>  					
			  						...
			  						<c:forEach items="${optList }" var="opt">
			  							<c:if test="${vo.basketNo == opt.basketNo }">		  								
			  								...
			  								<c:if test="${!empty opt.optName }">
						  						<!-- 상품 옵션 -->
						  						<h5><span class="badge badge-pill badge-secondary optionItem" data-optno="${opt.optNo }" data-optionprice=${opt.optionPrice } data-amount="${opt.amount }">
						  							옵션</span> ${opt.optName }
						  						</h5>
					  						</c:if>
							...
							<li class="list-group-item sellerItem">
								...
								<div class="row">
									...
									<div class="col-md-6" style="border-left: 1px solid #bbb;">
										<h5 class="ml-4"><b>주문 금액 <span class="text-primary sellerOrderPrice">0</span>원</b></h5>
									</div>
								</div>
							</li>
						</c:if>
