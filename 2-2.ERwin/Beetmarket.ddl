...
(
 ...
 NULL ,
	status                VARCHAR2(6)   DEFAULT  '정상' NULL  CHECK (status in('정상','탈퇴','후면','강퇴')),
...
(
	...
	orderState            VARCHAR2(30)   DEFAULT  '결제완' NOT NULL  CHECK (orderState in('결제완료','배송준비','배송중','배송완료','구매확정','취소요청','반품요청','요청처리')
),
	...
	goodsOptNo            NUMBER(13)  NULL ,
 PRIMARY KEY (orderNo),
 FOREIGN KEY (stockNo) REFERENCES pointShopStock(stockNo),
 FOREIGN KEY (id) REFERENCES member(id) ON DELETE CASCADE,
 FOREIGN KEY (goodsOptNo) REFERENCES goodsOption(goodsOptNo) ON DELETE SET NULL,
